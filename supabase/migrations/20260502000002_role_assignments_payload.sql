-- E5-T5: Add role_assignments.payload column and extend start_game RPC
-- to accept imposter hints and distribute them per-imposter.
--
-- role_assignments.payload: JSONB bag for imposter-specific data.
--   For imposters: {"hints": ["clue 1", "clue 2"]}
--   For civilians: {}
--
-- start_game now accepts p_hints text[] (default '{}').
--   The client (host) passes a flat array of hints where hints for
--   imposter[i] occupy indices [i*hint_count .. (i+1)*hint_count - 1].
--   hint_count is read from config_snapshot->>'imposter_hint_count'.
--   If p_hints is empty or hint_count = 0, payload stays {}.
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. Replace start_game with hint-aware version
-- ─────────────────────────────────────────────────────────────────────────────

DROP FUNCTION IF EXISTS public.start_game(uuid, text, integer, text);
